...
class Gender {
  ...
  static init = (genderId) => {
    ...
    this.list.forEach(
      (item) => (document.getElementById(item.id).checked = item.value == GENDER_ID),
    );
  };
...
function checkParams(
  ...
  if (!isAdded && !ID) return displayLog("L'identifiant est incorrect !", toConsole, prefixMsg);
  ...
  if (isAdded && !EMAIL) return displayLog("L'e-mail est invalide !", toConsole, prefixMsg);
  ...
  if (!isAdded) {
    ...
    if (!USER) return displayLog("L'identifiant est incorrect !", toConsole, prefixMsg);
...
function addAdmin(
  ...
  // Get the parameters
  const PARAMS = checkParams(true, name, email, password, pswdConfirm, toConsole, PREFIX_LOG);
  if (!PARAMS) return false;
...
function updateRole(userId, role, toConsole = true) {
  ...
  const ID = parseInt(userId);
  if (!ID) return displayLog("L'identifiant est incorrect !", toConsole, PREFIX_LOG);
  ...
  if (!USER) return displayLog("L'identifiant est incorrect !", toConsole, PREFIX_LOG);
