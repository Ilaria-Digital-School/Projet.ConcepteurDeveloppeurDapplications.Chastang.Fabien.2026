...
Array.prototype.lsAddItem = function (nameArray, nameId, item) {
  if (item[nameId] === 0) {
    ...
    console.log(
      `The item ID is not 0 (ID is ${item[nameId]}): the item cannot be added to local storage.`,
    );
};
...
Array.prototype.lsRemoveItem = function (nameArray, nameId, id) {
  ...
  if (INDEX > -1) {
    ...
    if (this.length == 0) localStorage.removeItem(nameArray);
    else this.lsSetItems(nameArray);

    console.log("The item has been removed from local storage.");
  } else console.log("The item to be removed does not exist in local storage.");
};

// Array method to enable or disable an item from an array in local storage
Array.prototype.lsSetItemVisibility = function (
  nameArray,
  nameId,
  id,
  isVisible,
...
class Role {
  ...
  static validate = (role) => {
    ...
    return ROLE == Role.user || ROLE == Role.admin || ROLE == Role.superAdmin
      ? ROLE
      : null;
  };
}
...
function getLoggedIn() {
  return (
    parseInt(localStorage.getItem("sessionId")) ||
    parseInt(sessionStorage.getItem("sessionId")) ||
    0
  );
}
...
function logout(toConsole = false) {
  ...
  if (ADD_PRODUCT && !ADD_PRODUCT.className.includes("inactive"))
  ...
  if (PAGES.every((page) => !HREF.includes(page)))
...
HTMLInputElement.prototype.checkPositiveNumber = function (
  isInt,
  maxValue,
  defaultValue,
) {
...
`;
const LOGOUT =
  '<a href="javascript:logout()"><i class="fa-solid fa-power-off"></i>Déconnexion</a>';
const NAVBAR = [
  {
    pages: "index.html",
    link: '<a href="../index.html" title="Accueil" aria-label="Accueil"><i class="fa-regular fa-house"></i></a>',
    selected:
      '<span class="selected-page" title="Accueil" aria-label="Accueil"><i class="fa-regular fa-house"></i></span>',
  },
  {
    pages: "products.html",
  ...
  },
  {
    ...
    selected:
      '<span class="selected-page" title="Mon panier" aria-label="Mon panier"><i class="fa-solid fa-cart-arrow-down"></i></span><span id="cart-sticker"></span>',
  },
  {
    pages: "login.html",
    submenu: true,
  ...
  },
  {
    ...
    selected:
      '<span class="selected-page" title="Contactez-nous" aria-label="Contactez-nous"><i class="fa-regular fa-envelope"></i></span>',
  },
  {
    pages: "about.html",
    ...
    selected:
      '<span class="selected-page" title="À propos de My Shop" aria-label="À propos de My Shop"><i class="fa-solid fa-circle-info"></i></span>',
  },
];
...
function setHeader() {
  ...
  const isSelected = (item) =>
    item.pages.split(",").some((page) => HREF.includes(page));
  const getLink = (item) =>
    !isSelected(item)
      ? INDEX_PAGE
        ? item.link
        : item.link.replace("./pages/", "./")
      : item.selected;
  ...
  NAVBAR.forEach((item) => {
    ...
    } else setLi(item, getLink(item)); // Fill the navigation bar
  });
}
...
function init() {
  ...
  if (MAIN_INDEX) {
    ...
    const PROMO = document.querySelector(
      "#main-index section:last-of-type mark",
    );
    if (PROMO) {
    ...
    if (products.length > 0) {
      ...
      products.forEach((product) => {
        ...
        // Display the product
        PRODUCT.display(CONTAINER, "article-product", {
          text: '<i class="fa-regular fa-eye"></i>',
          attributes: [
            { name: "title", value: "Voir" },
            { name: "aria-label", value: "Voir" },
          ],
          callbackfn: (id) =>
      ...
      document.querySelector("main section:nth-of-type(3) h2").textContent =
  ...
  if (MAIN_PRODUCTS) {
    ...
    if (PRODUCTS.length > 0) {
      ...
      fillCarousel(PRODUCTS, URL_PARAMS.get("id"), {
        text: '<i class="fa-solid fa-cart-arrow-down"></i>',
        attributes: [
          { name: "title", value: "Acheter" },
          { name: "aria-label", value: "Acheter" },
        ],
      ...
      let activeItemId =
        "product-" +
        this.querySelector(".carousel-item.active").getAttribute(
          "data-item-id",
        );
  ...
  const ADD_PRODUCT = document.getElementById("navbar-add-product");
  if (isLoggedInAdmin()) ADD_PRODUCT.classList.remove("inactive");
