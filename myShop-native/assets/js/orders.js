...
function checkCart(userId = null) {
  ...
  if (INDEX == -1 || CARTS[INDEX].products.length == 0)
    return displayLog(
      `${INDEX == -1 ? "Invalid cart" : "Empty cart"}!`,
      true,
      "[order: check cart] - ",
    );
...
function addOrder(userId = null, toConsole = false) {
  ...
  if (ORDER.userId > 0) {
    ...
    // Error
    displayLog(
      "Échec lors de la confirmation de la commande !",
      toConsole,
      PREFIX_LOG,
    );
...
function displayOrders(userId = null) {
  ...
  const ORDERS = lsGetItemsByFilter(
    "orders",
    (order) => order.isVisible && order.userId == USER_ID,
  ).reverse();
  if (ORDERS.length == 0)
    return displayError(
  ...
  ORDERS.forEach((order, index) => {
    ...
    H2.querySelector("span").textContent =
      ...
      " à " +
      DATE.toLocaleTimeString(undefined, DATE_OPTIONS);
