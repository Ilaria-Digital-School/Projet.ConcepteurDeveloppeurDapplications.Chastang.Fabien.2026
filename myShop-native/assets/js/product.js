...
class Product {
  ...
  display(container, classProd, button, info = true, sticker = true) {
    ...
    if (button.attributes)
      button.attributes.forEach(
        (attribute) => (btn += ` ${attribute.name}="${attribute.value}"`),
      );
  ...
  iniCarousel(productId, index, carouselInner, container, button) {
    ...
    if (this.id == productId) activeProduct = DIV;
    else if (index == 0) activeProduct = DIV;
    DIV.setAttribute("data-item-id", this.id);

    const IMG = document.createElement("img");
    ["d-block", "w-100", "carousel-img"].forEach((css) =>
...
function addProduct(toConsole = false) {
  ...
  const DESCRIPTION = document
    .getElementById("description-product")
    .value.trim();
...
function updateProduct(
  ...
  if (PRODUCTS.length == 0)
    return displayLog(
      "Mise à jour impossible : aucun produit n'est enregistré !",
      toConsole,
      PREFIX_LOG,
    );
  ...
  isChanged ||= product.updateProperty(
    "description",
    DESCRIPTION,
  ...
  if (isChanged) {
    ...
    displayLog(
      "Aucune modification n'a été apportée à votre produit !",
      toConsole,
      PREFIX_LOG,
    );
...
function setProductVisibility(productId, isVisible, toConsole = true) {
  lsGetItems("products").lsSetItemVisibility(
    "products",
    "id",
    productId,
    isVisible,
  );
  (toConsole ? console.log : alert)(
    (toConsole ? "[visibility product] - " : "") +
      `Le produit a été ${isVisible ? "réactivé" : "archivé"}.`,
  );
}
...
function removeProduct(productId, toConsole = true) {
  ...
  (toConsole ? console.log : alert)(
    (toConsole ? "[remove product] - " : "") + "Le produit a été supprimé.",
  );
}
...
function fillCarousel(products, productId, button) {
  ...
  products.forEach((product, index) => {
    ...
    const ACTIVE_PRODUCT = PRODUCT.iniCarousel(
      PRODUCT_ID,
      index,
      ...CONTAINERS,
      button,
