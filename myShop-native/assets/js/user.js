...
class Gender {
  ...
  static get = (genderId) => {
    if (genderId) {
      ...
      return this.list.some((item) => item.value == GENDER_ID)
        ? GENDER_ID
        : this.other.value;
  ...
  static init = (genderId) => {
    ...
    this.list.forEach(
      (item) =>
        (document.getElementById(item.id).checked = item.value == GENDER_ID),
    );
  };
...
class Interests {
  ...
  static get = (interestIDs) => {
    if (Array.isArray(interestIDs) && interestIDs.length > 0) {
      ...
      return this.list.filter((item) =>
        INTEREST_IDS.some((param) => param == item.value),
      );
  ...
  static init = (interestIDs) => {
    ...
    this.list.forEach(
      (item) =>
        (document.getElementById(item.id).checked = INTEREST_IDS.some(
...
class Country {
  ...
  static get = (countryId) => {
    if (countryId) {
      ...
      return this.list.some((item) => item.value == COUNTRY_ID)
        ? COUNTRY_ID
        : this.other.value;
    } else return this.other.value;
  };
  ...
  update(name, email, password, genderId, interestIDs, countryId) {
    ...
    if (interestIDs) {
      ...
      isChanged ||= this.updateProperty(
        "interests",
        INTEREST_IDS,
        JSON.stringify(this.interests) != JSON.stringify(INTEREST_IDS),
      );
    }
    if (countryId) {
...
function checkParams(
  ...
  if (isAdded && !NAME)
    return displayLog(
      "Le nom doit contenir au moins trois caractères non blancs !",
      toConsole,
      prefixMsg,
    );
  ...
  if (isAdded && !PASSWORD)
    return displayLog(
      "Le mot de passe doit contenir au moins 8 caractères et aucun caractère blanc !",
      toConsole,
      prefixMsg,
    );
  ...
  if (!isAdded) {
    if (USERS.length == 0)
      return displayLog(
        "Mise à jour impossible : aucun utilisateur n'est enregistré !",
        toConsole,
        prefixMsg,
      );
  ...
  return isAdded
    ...
    : [NAME, EMAIL, PASSWORD, USERS, USER];
}
...
function addUser(countryId = 0, toConsole = false) {
  ...
  let objGender = document.querySelector(
    `input[name="${Gender.fieldName}"]:checked`,
  );
  const GENDER_ID = objGender ? Gender.get(objGender.value) : Gender.other;
  let objInterests = document.querySelectorAll(
    `input[name="${Interests.fieldName}"]:checked`,
  );
  const INTEREST_IDS = objInterests
    ? Interests.get(Array.from(objInterests).map((item) => item.value))
    : Interests.other;
  let objCountry = document.getElementById(Country.fieldId);
  const COUNTRY_ID = Country.get(
...
function updateUser(
  ...
  prefixMsg = toConsole
    ? `[update ${USER.role != Role.superAdmin ? (USER.role != Role.admin ? "user" : "admin") : "super-admin"}] - `
    : null;

  // Perform the update
  const IS_CHANGED = USER.update(
    NAME,
    EMAIL,
    PASSWORD,
    genderId,
  ...
  if (IS_CHANGED) {
    ...
    displayLog(
      "Aucune modification n'a été apportée à votre profile !",
      toConsole,
      prefixMsg,
    );
...
function setUserVisibility(userId, isVisible, toConsole = true) {
  ...
  displayLog(
    `Votre profile a été ${isVisible ? "réactivé" : "archivé"}.`,
    toConsole,
    toConsole ? "[visibility user] - " : null,
  );
}
...
function removeUser(userId, toConsole = true) {
  ...
  displayLog(
    "Votre profile a été supprimé.",
    toConsole,
    toConsole ? "[remove user] - " : null,
  );
}
...
function loginUser(userId, permanent) {
  if (permanent) localStorage.setItem("sessionId", userId);
  else sessionStorage.setItem("sessionId", userId);
}
function login(toConsole = false) {
  ...
  if (USER) {
    ...
    loginUser(
      USER.id,
      document.querySelector('input[id="permanent-login"]:checked'),
    );
...
function addAdmin(
  ...
  const PREFIX_LOG = toConsole
    ? `[add ${isSuper ? "super-admin" : "admin"}] - `
    : null;
...
function updateRole(userId, role, toConsole = true) {
  ...
  if (IS_CHANGED === null)
    return displayLog(
      "Le rôle de l'utilisateur est incorrect !",
      toConsole,
      PREFIX_LOG,
    );
  ...
  if (IS_CHANGED) {
    ...
    USERS.lsSetItems("users");
    displayLog(
      "Le rôle de l'utilisateur a été mis à jour.",
      toConsole,
      PREFIX_LOG,
    );
  } else
    // No changes have been done
    displayLog(
      "Aucune modification n'a été apporté au rôle de l'utilisateur !",
      toConsole,
      PREFIX_LOG,
    );
