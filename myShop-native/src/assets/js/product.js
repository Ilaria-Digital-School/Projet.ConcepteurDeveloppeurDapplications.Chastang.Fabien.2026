...
class Product {
  ...
  display(container, classProd, button, info = true, sticker = true) {
    ...
    if (button.attributes)
      button.attributes.forEach((attribute) => (btn += ` ${attribute.name}="${attribute.value}"`))
...
function addProduct(toConsole = false) {
  ...
  if (!PRICE) return displayLog('Prix doit être un nombre positif !', toConsole, PREFIX_LOG)
