...
function updateCartProduct(productId, quantity, userId = null) {
  ...
  if (FOUND) {
    ...
    if (CART.updateQuantity(PRODUCT_ID, QUANTITY)) {
      ...
      return true
    } else return displayLog('The product is not in the cart!', true, '[cart: update product] - ')
  } else return displayLog('The user does not have a cart!', true, '[cart: update product] - ')
}
...
function removeCartProduct(productId, userId = null) {
  ...
  if (FOUND > -1) {
    ...
    if (CART.removeProduct(PRODUCT_ID)) {
      ...
      return true
    } else return displayLog('The product is not in the cart!', true, '[cart: remove product] - ')
  } else return displayLog('The user does not have a cart!', true, '[cart: remove product] - ')
}
...
function displayCart(userId = null) {
  ...
  // Retrieve the user's cart
  const [CARTS, FOUND] = lsFind('carts', (cart) => cart.userId == USER_ID)
  if (!FOUND) return displayError('Votre panier est vide', 'h2-info inactive', 'h2-title')
  ...
  if (TOTAL_PRICE == 0) return displayError('Votre panier est vide', 'h2-info inactive', 'h2-title')
