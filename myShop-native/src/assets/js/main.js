...
HTMLInputElement.prototype.checkPositiveNumber = function (isInt, maxValue, defaultValue) {
  ...
  this.value =
    value > 0 ? (value <= maxValue ? value : maxValue) : defaultValue ? defaultValue : '';
};
...
function init() {
  ...
  if (MAIN_PRODUCTS) {
    ...
    if (PRODUCTS.length > 0) {
      ...
      let activeItemId =
        'product-' + this.querySelector('.carousel-item.active').getAttribute('data-item-id');
  ...
  else if (!ADD_PRODUCT.className.includes('inactive')) ADD_PRODUCT.classList.add('inactive');
